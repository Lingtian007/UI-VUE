...
export default {
  ...
  data () {
    return {
      ...
      label: '',
      Route: 'actionapi/ItemGroup/QueryItemGroup',
      parameter: ''
    }
  },
  created () {
    ...
    if (this.Types === 'eqia_r') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqia_r'}
    } else if (this.Types === 'eqiw_d') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqiw_d'}
    } else if (this.Types === 'eqia_p') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqia_p'}
    } else if (this.Types === 'eqia_rd') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqia_rd'}
    } else if (this.Types === 'eqiw_r') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqiw_r'}
    } else if (this.Types === 'eqiw_l') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqiw_l'}
    } else if (this.Types === 'eqiw_lt') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqiw_lt'}
    } else if (this.Types === 'eqin_t') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqin_t'}
    } else if (this.Types === 'eqin_f') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqin_f'}
    } else if (this.Types === 'eqin_a') {
      this.parameter = {'UserID': '578', 'modeltype': 'eqin_a'}
    } else {
      this.parameter = {'UserID': '578', 'modeltype': 'eqia_r'}
    }
    this.getData()
  ...
  methods: {
    ...
    getData (val) {
      if (this.itemGroup === 'group') { 
        axios.post(`${this.url}` + 'actionapi/ItemGroup/QueryItemGroup', this.parameter).then((res) => {
        ...
        axios.post(`${this.url}` + 'actionapi/Item/GetItemAll', parmars).then((res) => {
          console.log('当前数据 res==== ', res)
          res.data.data.map((item, index) => {
            this.opintdata.push({
              label: item.fldName,
